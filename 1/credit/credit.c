/**
 * Credit: Implement a program that prompts the user for a credit card number and then reports (via
 *     `printf`) whether it is a valid American Express, MasterCard, or Visa card number, per the
 *     definitions of each's format herein. So that we can automate some tests of your code, we ask
 ...
 *     will be entirely numeric (i.e., devoid of hyphens, as might be printed on an actual card) and
 *     that it won’t have leading zeroes. But do not assume that the user’s input will fit in an
 *     `int`!
 */
